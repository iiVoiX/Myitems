...
public class LoreStatsManager extends HandlerManager {
    ...
    public final String statsValue(final LoreStatsEnum loreStats, double value1, double value2) {
        ...
        if (loreStats.equals(LoreStatsEnum.DURABILITY)) {
            ...
            value1 = MathUtil.limitDouble(value1, 1.0, maxLevel);
        ...
        if (loreStats.equals(LoreStatsEnum.DAMAGE)) {
            ...
            statsValue = ((value1 > 0.0) ? (positiveValue + "+" + MainConfig.KEY_STATS_VALUE + textValue1 + "%" + MainConfig.KEY_STATS_VALUE + colorValue) : (MainConfig.KEY_STATS_VALUE + textValue1 + "%" + MainConfig.KEY_STATS_VALUE + colorValue));
    ...
    public boolean durability(final LivingEntity livingEntity, final Slot slot, final int currentValue) {
        if (EntityUtil.isPlayer(livingEntity)) {
            ...
            return item == null || item.getType().equals(Material.BOW) || this.durability(livingEntity, item, currentValue, false);
        }
        return true;
    }
    ...
    public final void sendBrokenCode(final LivingEntity livingEntity, final Slot slot, final boolean broken) {
        ...
        if (EntityUtil.isPlayer(livingEntity)) {
            ...
            switch (slot) {
                case MAINHAND: {
                    message = lang.getText(player, "Item_Broken_MainHand");
                    break;
                }
                case OFFHAND: {
                    message = lang.getText(player, "Item_Broken_Offhand");
                    break;
                }
                case HELMET: {
                    message = lang.getText(player, "Item_Broken_Helmet");
                    break;
                }
                case CHESTPLATE: {
                    message = lang.getText(player, "Item_Broken_Chestplate");
                    break;
                }
                case LEGGINGS: {
                    message = lang.getText(player, "Item_Broken_Leggings");
                    break;
                }
                case BOOTS: {
                    message = lang.getText(player, "Item_Broken_Boots");
                    break;
                }
                default: {
                    return;
