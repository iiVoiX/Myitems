        ...
        for (int length = (values = Dependency.values()).length, i = 0; i < length; ++i) {
            ...
            for (final String dependency : dependencyManager.getDependency(type)) {
                if (Bukkit.getPluginManager().isPluginEnabled(dependency)) {
                    ...
                    if (dependencyManager.getTypeDependency(dependency).equals(Dependency.HARD_DEPENDENCY)) {
        ...
        if (dependencyManager.hasAnyDependency()) {
            ...
            for (int length = (values = Dependency.values()).length, i = 0; i < length; ++i) {
                ...
                if (dependencyManager.hasDependency(type)) {
                    final MessageBuild messageListHeader = lang.getMessage(type.equals(Dependency.HARD_DEPENDENCY) ? "Hook_Hard_Dependency_Header" : "Hook_Soft_Dependency_Header");
                    ...
                    for (final String hook : dependencyManager.getDependency(type)) {
                        ...
                        final MessageBuild messageList = lang.getMessage(type.equals(Dependency.HARD_DEPENDENCY) ? "Hook_Hard_Dependency_Header" : "Hook_Soft_Dependency_List");
